    ...
    V: EngineValidator<T>,
{
    ...
    fn on_forkchoice_updated(
        ...
        debug!(target: "engine::tree", ?attrs, "invoked forkchoice update");
        ...
        if self.state.tree_state.canonical_block_hash() == state.head_block_hash {
            debug!(target: "engine::tree", "fcu head hash is already canonical");
        ...
        if let Some(chain_update) = self.on_new_head(state.head_block_hash)? {
            ...
            debug!(target: "engine::tree", tip = ?tip, "ensured consistent forkchoice state");

            if let Some(attr) = attrs {
                let updated = self.process_payload_attributes(attr, &tip, state, version);
                debug!(target: "engine::tree", tip = ?tip, "processed payload attributes");
                return Ok(TreeOutcome::new(updated));
            }
    ...
    fn try_recv_engine_message(
        ...
        if self.persistence_state.in_progress() {
            ...
            debug!(target: "engine::tree", "persistent state is in progress, wait max 500ms for it complete");
            match self.incoming.recv_timeout(std::time::Duration::from_millis(500)) {
                Ok(msg) => {
                    debug!(target: "engine::tree", "received new engine message while persistence is in progress");
                    Ok(Some(msg))
                }
                Err(err) => match err {
            ...
            debug!(target: "engine::tree", "persistence state is not in progress, receiving new engine message");
    ...
    fn advance_persistence(&mut self) -> Result<(), AdvancePersistenceError> {
        debug!(target: "engine::tree", "Advancing persistence state");
        if self.persistence_state.in_progress() {
            debug!(target: "engine::tree", "persistence state is in progress, waiting for response");
        ...
        debug!(target: "engine::tree", "Checking if persistence state is in progress");
        if !self.persistence_state.in_progress() {
            if let Some(new_tip_num) = self.find_disk_reorg()? {
                self.remove_blocks(new_tip_num)
            } else if self.should_persist() {
                debug!(target: "engine::tree", "getting canonical blocks to persist");
                let blocks_to_persist = self.get_canonical_blocks_to_persist()?;
                debug!(target: "engine::tree", "persisting blocks");
                    ...
                    EngineApiRequest::Beacon(request) => {
                        match request {
                            BeaconEngineMessage::ForkchoiceUpdated {
                                ...
                                debug!(target: "engine::tree", "Sending forkchoice updated response");
                                if let Err(err) =
    ...
    fn on_canonical_chain_update(&mut self, chain_update: NewCanonicalChain<N>) {
        ...
        debug!(target: "engine::tree", tip = ?chain_update.tip().num_hash(), "updated canonical head");

        let tip = chain_update.tip().clone_sealed_header();
        debug!(target: "engine::tree", "cloned sealed header");
        let notification = chain_update.to_chain_notification();
        debug!(target: "engine::tree", "created chain notification");
        ...
        debug!(target: "engine::tree", "start update chain");
        // update the tracked in-memory state with the new chain
        self.canonical_in_memory_state.update_chain(chain_update);
        debug!(target: "engine::tree", "updated in-memory state");
        self.canonical_in_memory_state.set_canonical_head(tip.clone());
        debug!(target: "engine::tree", "updated canonical head");
        ...
        debug!(target: "engine::tree", tip = ?tip, "notified canonical state");
    ...
    fn process_payload_attributes(
        ...
        match <T::PayloadBuilderAttributes as PayloadBuilderAttributes>::try_new(
            ...
            Ok(attributes) => {
                ...
                debug!(target: "engine::tree", "Sending new payload to payload builder");
