    ...
    N: NodeTypesWithDB<DB: fmt::Debug, ChainSpec: fmt::Debug, Storage: fmt::Debug>,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self { db, chain_spec, static_file_provider, prune_modes, storage, storage_settings, rocksdb_provider } =
            self;
...
mod tests {
    ...
    fn provider_factory_with_database_path() {
        ...
        let temp_dir = tempfile::TempDir::new().expect(ERROR_TEMPDIR).keep();
        
        // Initialize RocksDB provider for tests
        let rocksdb_path = temp_dir.path().join("rocksdb");
        let rocksdb_provider = crate::providers::RocksDBProvider::builder(&rocksdb_path)
            .with_metrics()
            .with_statistics()
            .build()
            .expect("Failed to initialize RocksDB for test");
